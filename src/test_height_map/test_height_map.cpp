...
//    gModelRope = std::make_shared<Model>(RES_DIR PATH_SEP "crane/rope.obj");
    ...
    shader = std::make_shared<Shader>(
            ROOT_PATH PATH_SEP "src" PATH_SEP "test_height_map" PATH_SEP "material.vert",
            ROOT_PATH PATH_SEP "src" PATH_SEP "test_height_map" PATH_SEP "material.frag"
    );
...
void draw()
{
    ...
    for (int i = 0; i < height; ++i) {
        ...
        for (int j = 0; j < width; ++j) {
            ...
            float z = data[i * width + j] / 3500.0f;
            glVertex3f(x, y, z);
        }
        glEnd();
    }
...
{
    int chann;
    data = stbi_load_16(RES_DIR PATH_SEP "height_short_mat_5.png", &width, &height, &chann, 0);
}